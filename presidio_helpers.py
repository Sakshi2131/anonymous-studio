...
def nlp_engine_and_registry(
    ...
    # Local import to avoid cyclic dependency at module import time
    import presidio_nlp_engine_config

    # Set up NLP Engine according to the model of choice
    if "spacy" in model_family.lower():
        return presidio_nlp_engine_config.create_nlp_engine_with_spacy(model_path)
    if "stanza" in model_family.lower():
        return presidio_nlp_engine_config.create_nlp_engine_with_stanza(model_path)
    elif "flair" in model_family.lower():
        return presidio_nlp_engine_config.create_nlp_engine_with_flair(model_path)
    elif "huggingface" in model_family.lower():
        return presidio_nlp_engine_config.create_nlp_engine_with_transformers(
            model_path
        )
    elif "azure ai language" in model_family.lower():
        return presidio_nlp_engine_config.create_nlp_engine_with_azure_ai_language(
            ta_key, ta_endpoint
        )
    else:
...
def call_openai_api(
    ...
    fake_data = call_completion_model(prompt=prompt, openai_params=openai_params)
    return fake_data
